...
def hamming_normalized(vector1, vector2):
    ...
    return float(np.mean(np_vector1 != np_vector2))
...
def cosine(vector1, vector2):
    ...
    if np.linalg.norm(np_vector1) == 0 or np.linalg.norm(np_vector2) == 0:
        raise ValueError("vectors norm cant be zero")
...
def canberra(vector1, vector2):
    ...
    summandlist = []
    for i in range(len(np_vector1)):
        if np.abs(np_vector1[i]) + np.abs(np_vector2[i]) != 0:
            summand = (np.abs(np_vector1[i] - np_vector2[i])) / (np.abs(np_vector1[i]) + np.abs(np_vector2[i]))
            summandlist.append(summand)
    canberra_dis = np.sum(summandlist)
